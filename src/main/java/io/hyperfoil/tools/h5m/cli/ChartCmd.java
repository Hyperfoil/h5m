...
public class ChartCmd implements Command<H5mCommandInvocation>, FolderAware {
    ...
    public CommandResult execute(H5mCommandInvocation invocation) throws InterruptedException {
        ...
        controlsLine = CYAN + "← → scroll  |  Home/End jump  |  f fingerprint  |  q quit" + CYAN;
    ...
    LineChart buildChart() {
        ...
        addDetectionMarkers(chart, nodeGroup, seriesList, fingerprintNode);
        return chart;
    }
    ...
    /**
     * A plotted series plus the X tick label for each of its points,
     * aligned by position; {@code rows} are the chart rows backing it and
     * {@code fingerprintOrNull} the selected fingerprint (null = merged).
     * Labels come from the row via {@code labeler} (domain value or upload
     * timestamp depending on mode).
     */
    private record SeriesData(DataSeries series, List<String> xLabels,
                              List<JqValue> rows, String fingerprintOrNull) {
    }
    ...
    private SeriesData buildSeries(String name, List<JqValue> values,
        ...
        List<JqValue> keptRows = new ArrayList<>();
        ...
        return new SeriesData(series, xLabels, keptRows, fingerprintOrNull);
    }
    ...
    private void addDetectionMarkers(LineChart chart, NodeGroup nodeGroup,
        ...
        Map<Long, Long> rootBySource = valueServiceImpl.rootIdsForValues(sourceIds);
        Map<SeriesData, Map<Long, Integer>> indexBySeries = new IdentityHashMap<>();
        for (PlacedMarker match : matches) {
            indexBySeries.computeIfAbsent(match.series(), s -> rootIndexByRoot(s.rows()));
        }

        for (PlacedMarker match : matches) {
            Map<Long, Integer> indexByRoot = indexBySeries.get(match.series());
            Integer x = null;
            for (Long sourceId : sourcesByDetection.get(match.detValue())) {
            ...
            if (x == null) continue;
            Marker marker = markerForDetection(match.node().type(), match.detValue().data(), x);
            if (marker != null) {
    ...
    private Map<Long, Integer> rootIndexByRoot(List<JqValue> rows) {
        Map<Long, Integer> indexByRoot = new HashMap<>();
        for (int i = 0; i < rows.size(); i++) {
            JqValue root = rows.get(i).getField(ValueService.ROOT_FIELD);
            if (root == null || root.isNull()) continue;
            Double id = root.tryDouble();
            if (id != null) {
                indexByRoot.putIfAbsent(id.longValue(), i);
            }
        }
        return indexByRoot;
    }
    ...
    /**
     * Build a marker for one detection at series position {@code x}.
     * Returns null when the detection carries no plottable value.
     */
    private Marker markerForDetection(NodeType type, JqValue data, int x) {
        Double y;
        String label;
        char symbol;
        String color;
        ...
        if (y != null) {
            return Marker.at(x, y).label(label).color(color).symbol(symbol);
        }
        return null;
    }
