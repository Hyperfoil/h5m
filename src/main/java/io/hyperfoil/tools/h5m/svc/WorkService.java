...
public class WorkService implements WorkServiceInterface {
    ...
    public void execute(Work w){
        ...
        try {
            ...
            if(w.notify && work.activeNode.type().isDetection() && !newOrUpdated.isEmpty()){
            ...
            if(!newOrUpdated.isEmpty()){
                if(work.activeNode!=null){
                    create(nodeService.getDependentNodes(work.activeNode).stream().map(node -> new Work(node, node.sources, work.sourceValues).setNotify(w.notify)).toList());
                }
            }
